...
const Info = () => {
  ...
  const calculateTargetPosition = () => {
    ...
    const targetCenterY =
      targetRect.top - wrapperRect.top + targetRect.height / 3;
  ...
  const handleUpdate = () => {
    ...
    const rawP = Math.max(
      0,
      (scrollY - (sectionTop + animationStartOffset)) / animationRange
    );
    ...
    const topNow =
      initialPosition.top +
      (targetPosition.top -
        (startWidthPx * endScale) / 2 -
        initialPosition.top) *
        ease;
    const leftNow =
      initialPosition.left +
      (targetPosition.left - initialPosition.left) * ease;
    ...
    <InfoWrapper ref={wrapperRef}>
      ...
      <InfoCardRow>
        <InfoCard>
          <div>Date</div>
          <div>
            2025.09.12(금) 18:00 - 21:00
            <br />
            2025.09.13(토) 11:00 - 16:20
          </div>
        </InfoCard>
        <InfoCard>
          <div>Location</div>
      ...
      <InfoContent>
        ...
        <div>
          제11회 국제한국학포럼에 참여해 주신 모든 분들께 진심으로 환영의 인사를
          전합니다. <br />
          <br />
          국제한국학포럼은 국내 최초의 학생 주도 융합형 한국학 포럼으로, 지난
          10년간 다양한 주제를 통해 한국의 사회, 문화, 정치, 외교를 다층적으로
          탐구해왔습니다. 올해 새로운 10년의 첫걸음을 내딛는 제11회 포럼에서는
          'AI 혁명과 한국'이라는 대주제 아래, 인공지능이라는 거대한 전환의 흐름
          속에서 한국이 직면한 도전과 기회를 함께 논의하고자 합니다. <br />
          <br />
          인공지능은 이미 우리의 삶 곳곳에 깊숙이 침투해 사회 전반에 새로운
          기준과 방향을 요구하고 있습니다. 디지털 문명사적 전환기라 불릴 만큼
          급격한 변화 속에서, 한국은 어떤 정체성을 지켜가며 어떠한 미래를
          설계해야 할지에 대한 질문은 그 어느 때보다 절실합니다. 이번 포럼에서
          준비한 3개의 부스와 5개의 세션은 그러한 고민을 청년의 시선에서
          자유롭고 비판적으로 성찰하고, 다양한 분야의 전문가들과 함께 해법을
          모색하는 장이 될 것입니다. <br />
          <br />
          제11회 국제한국학포럼 사무국은 'AI 혁명과 한국'이라는 방대한 주제를
          진정성 있게 담아내기 위해 지난 수개월간 치열하게 고민하며 준비해
          왔습니다. 이번 포럼이 한국학의 지평을 확장하는 동시에, 청년 세대의
          지적 담론이 사회적 변화로 이어질 수 있는 계기가 되기를 바랍니다. 바쁜
          일정 속에서도 참여해주신 모든 분들께 다시 한 번 감사의 인사를
          전합니다. <br />
          <br />
          감사합니다.
        </div>
      ...
      >
        <InfoAnchorCard>
          <InfoAnchorText>
            ...
            >
              Ready to Join the GKSF?
            </div>
            <div style={{ opacity: 0.8 }}>
              AI 시대, 한국학의 방향은 어디로 향할까요?
              <br />
              글로벌한 관점에서 질문하고, 함께 논의해요.
            </div>
          </InfoAnchorText>

          <InfoAnchorButton onClick={() => navigate('/about')}>
            About GKSF
          </InfoAnchorButton>
        </InfoAnchorCard>
      </InfoAnchorArea>
